    ...
    sfVector2u window_size)
{
    ...
    settings_menu->button[0] = create_button(button_info, &increase_global_volume, PLUS);

    button_info.position = (sfVector2f){
    half - quarter - TEXT_BUTTON_WIDTH / 2 - button_info.size.x + 2, 420};
    settings_menu->button[1] = create_button(button_info, &decrease_global_volume, MINUS);
    return;
}
    ...
    sfVector2u window_size)
{
    ...
    settings_menu->button[2] = create_button(button_info, &increase_music_volume, PLUS);

    button_info.position = (sfVector2f){
    half - quarter - TEXT_BUTTON_WIDTH / 2 - button_info.size.x + 2, 610};
    settings_menu->button[3] = create_button(button_info, &decrease_music_volume, MINUS);
    return;
}
    ...
    sfVector2u window_size)
{
    ...
    settings_menu->button[7] = create_button(button_info, &nothing, LOW_RESOLUTION);

    button_info.position =
        (sfVector2f){half + quarter - 372 / 4 * 2.9 / 2 + 190, 610};
    settings_menu->button[8] = create_button(button_info, &nothing, HIGH_RESOLUTION);
    ...
    object_info_t button_info, sfVector2u window_size)
{
    ...
    settings_menu->button[4] = create_button(button_info, &toggle_fullscreen, FULLSCREEN);

    button_info.position =
    (sfVector2f){half + quarter - 372 / 4 * 2.9 / 2 + 190, 420};
    settings_menu->button[5] = create_button(button_info, &toggle_windowed, WINDOWED);

    button_info.position =
    (sfVector2f){half + quarter - 372 / 4 * 2.9 / 2, 320};
    settings_menu->button[6] = create_button(button_info, &toggle_borderless, BORDERLESS);
