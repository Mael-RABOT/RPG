...
void update_settings_menu(app_t *app)
{
    sfVector2u window_size = sfRenderWindow_getSize(app->window);
    float half = window_size.x / 2;
    float quarter = window_size.x / 4;

    sfSprite_setScale(app->settings_menu->text_sounds->sprite, (sfVector2f){5, 5});
    sfSprite_setPosition(app->settings_menu->text_sounds->sprite, (sfVector2f){half - quarter - TITLE_WIDTH / 2, 120});
    sfSprite_setPosition(app->settings_menu->text_resolution->sprite, (sfVector2f){half + quarter - TITLE_WIDTH / 2, 120});
    sfSprite_setScale(app->settings_menu->text_resolution->sprite, (sfVector2f){5, 5});
    sfSprite_setScale(app->settings_menu->sounds->sprite, (sfVector2f){3, 3});
    sfSprite_setPosition(app->settings_menu->sounds->sprite, (sfVector2f){half - quarter - TEXT_BUTTON_WIDTH / 2, 420});
    sfSprite_setScale(app->settings_menu->music->sprite, (sfVector2f){3, 3});
    sfSprite_setPosition(app->settings_menu->music->sprite, (sfVector2f){half - quarter - TEXT_BUTTON_WIDTH / 2, 610});
    return;
}
