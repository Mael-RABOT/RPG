...
sfRenderWindow *window)
{
    ...
    sfSprite_setPosition(settings_menu->text_sounds->sprite,
    (sfVector2f){half - quarter - TITLE_WIDTH / 2, 120});
    sfSprite_setPosition(settings_menu->text_resolution->sprite,
    (sfVector2f){half + quarter - TITLE_WIDTH / 2, 120});
    sfSprite_setScale(settings_menu->text_resolution->sprite,
    (sfVector2f){5, 5});
    sfSprite_setScale(settings_menu->sounds->sprite, (sfVector2f){3, 3});
    sfSprite_setPosition(settings_menu->sounds->sprite,
    (sfVector2f){half - quarter - TEXT_BUTTON_WIDTH / 2, 420});
    sfSprite_setScale(settings_menu->music->sprite, (sfVector2f){3, 3});
    sfSprite_setPosition(settings_menu->music->sprite,
...
void display_settings_menu(sfRenderWindow *window, settings_menu_t *menu)
{
    ...
    sfRenderWindow_drawRectangleShape(window, menu->sounds_more->rect, NULL);
    sfRenderWindow_drawRectangleShape(window, menu->sounds_less->rect, NULL);
    sfRenderWindow_drawRectangleShape(window, menu->music_more->rect, NULL);
    sfRenderWindow_drawRectangleShape(window, menu->music_less->rect, NULL);
    sfRenderWindow_drawRectangleShape(window, menu->full_screen->rect, NULL);
    sfRenderWindow_drawRectangleShape(window, menu->windowded->rect, NULL);
